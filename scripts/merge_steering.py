...
Examples:
  ...
  python scripts/merge_steering.py \\
    ...
    # Mode selection
    parser.add_argument(
        "--verify", action="store_true", help="Verify a previously merged model instead of merging"
    )

    # Model paths
    parser.add_argument(
        "--model", "--original-model", help="Path to base/original HuggingFace model"
    )
    parser.add_argument("--merged-model", help="Path to merged model (for verification)")

    # Steering configuration
    parser.add_argument(
        "--steering-vectors", required=False, help="Path to steering vectors .pt file"
    )
    parser.add_argument(
        "--alpha",
        type=float,
    ...
    layer_group.add_argument(
        "--layers", type=int, nargs="+", help="Specific layer indices to modify"
    )
    layer_group.add_argument(
        "--correlations", help="Path to layer_correlations.json (to use with --top-k)"
    )
    ...
    # Output
    parser.add_argument("--output-dir", "--output", help="Directory to save merged model")

    # Component selection
    parser.add_argument(
        "--component",
        default="mlp",
        help="Which component(s) to merge: mlp, attn, or attn+mlp (default: mlp)",
    )
    parser.add_argument(
        "--alpha-attn",
        type=float,
        default=None,
        help="Override alpha for attention component (default: use --alpha)",
    )
    parser.add_argument(
        "--alpha-mlp",
        type=float,
        default=None,
        help="Override alpha for MLP component (default: use --alpha)",
    )
    ...
    if args.merged_model:
        args.merged_model = resolve_model_path(args.merged_model)

    # Verification mode
    if args.verify:
        if not args.merged_model or not args.model or not args.layers:
            print(
                "[ERROR] Verification requires --merged-model, --model, and --layers",
                file=sys.stderr,
            )
            return 1
        ...
        results = verify_merged_model(
            model_dir=args.merged_model, original_model_path=args.model, steering_layers=args.layers
        )
